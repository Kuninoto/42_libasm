...
while:
    cmp BYTE[rdi], 0x00 ; Compare the byte in rdi with 0 (to check if it is a null byte)
    je return ; Jump if condition evaluates to true

    cmp BYTE[rsi], 0x00 ; Compare the byte in rsi with 0 (to check if it is a null byte)
    je return ; Jump if condition evaluates to true
